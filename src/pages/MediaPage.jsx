...
import { C, MEDIA, LEVELS, LEVEL_THEME } from '../lib/constants'
...
export default function MediaPage({ user, embedded = false }) {
  ...
  const th=LEVEL_THEME[lv]||LEVEL_THEME.A1
  return (
    <div style={embedded ? {} : {flex:1,overflow:'auto',padding:'16px 18px'}}>
      ...
      <div style={{display:'flex',gap:4,marginBottom:14}}>
        {LEVELS.map(l=>{const lt=LEVEL_THEME[l];const on=lv===l;return(
          <button key={l} onClick={()=>setLv(l)} style={{flex:1,padding:'6px',borderRadius:7,border:`1.5px solid ${on?lt.main:C.border}`,background:on?lt.main:'transparent',color:on?lt.on:C.textS,cursor:'pointer',fontSize:11,fontWeight:600,fontFamily:'inherit'}}>
            {l}
          </button>
        )})}
      </div>
      {(MEDIA[lv]||[]).map((m,i)=>(
        <a key={i} href={m.url} target="_blank" rel="noopener noreferrer" style={{textDecoration:'none'}}>
          <div style={{display:'flex',alignItems:'center',gap:10,padding:'12px 13px',background:'#fff',borderRadius:11,border:`1px solid ${C.border}`,marginBottom:8,cursor:'pointer',transition:'box-shadow .15s'}}
            ...
            <div style={{width:36,height:36,borderRadius:8,background:th.light,display:'flex',alignItems:'center',justifyContent:'center',fontSize:18,flexShrink:0}}>{ICONS[m.type]||'▶️'}</div>
            <div style={{flex:1}}>
              ...
              <Badge label={m.platform} color={th.main} bg={th.light}/>
            </div>
            <span style={{color:th.main,fontSize:14}}>↗</span>
          </div>
        </a>
      ))}
    </div>
