...
export default function CurriculumPage({ user, completedTopics, onOpenLesson }) {
  ...
  return (
    <div style={{flex:1,overflow:'auto',padding:'16px 18px'}}>
      ...
      {LEVELS.map(lv=>{
        const topics=CURRICULUM[lv];const done=topics.filter(t=>completedTopics?.includes(t.id)).length;const isCur=lv===user?.level;const isExp=exp===lv;const th=LEVEL_THEME[lv]
        return(
          <div key={lv} style={{background:'#fff',borderRadius:13,border:`1px solid ${C.border}`,borderLeft:`4px solid ${th.main}`,padding:'13px 16px',marginBottom:9,cursor:'pointer'}} onClick={()=>setExp(isExp?null:lv)}>
            <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:isExp?11:0}}>
              ...
              <div style={{flex:1}}>
                ...
                <PBar pct={(done/topics.length)*100} color={th.main} h={4}/>
